...
int main(int argc, char *argv[]) {
  ...
  for(; a < argc; a++){
    if(strcmp(argv[a], "-f") == 0){
      ...
      fd = open(argv[a], O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
      if(fd == -1){
